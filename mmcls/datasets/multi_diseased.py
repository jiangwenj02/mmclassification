...
def has_file_allowed_extension2(filename, extensions):
...
def get_samples2(root, folder_to_idx, extensions):
    ...
    for folder_name in sorted(list(folder_to_idx.keys())):
        ...
        for _, _, fns in sorted(os.walk(_dir)):
            for fn in sorted(fns):
                print(fn)
                if has_file_allowed_extension2(fn, extensions):
...
class MultiDiseased(ImageNet):
    ...
    def load_annotations(self):
        if self.ann_file is None:
            folder_to_idx = find_folders2(self.data_prefix)
            print(folder_to_idx)
            samples = get_samples2(
                self.data_prefix,
                folder_to_idx,
